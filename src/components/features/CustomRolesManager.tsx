                                    ...
                                    {group.keys.map(perm => (
                                        <label key={perm} className={`flex items-center gap-2 ${readOnly ? "cursor-default" : "cursor-pointer"}`}>
                                            <div onClick={() => !readOnly && toggle(perm)}
                                                className={`w-4 h-4 rounded flex items-center justify-center border transition-colors ${selected.includes(perm) ? "bg-brand-blue border-brand-blue" : "border-slate-300"} ${readOnly ? "" : "cursor-pointer"}`}>
            ...
            {!readOnly && (
                <div className="flex gap-2 pt-1">
                    <button type="submit" disabled={pending}
                        className="flex-1 px-3 py-2 rounded-xl bg-brand-blue text-white text-sm font-black disabled:opacity-50">
