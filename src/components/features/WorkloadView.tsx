            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                        <span className="text-[11px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-black uppercase tracking-widest">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => setOffset(0)}
                            className="px-2 py-1 rounded-lg text-[11px] font-black text-slate-500 hover:bg-slate-100 transition-colors">
                            Today
                        </button>
                        <button onClick={() => setOffset(o => o + 1)}
                            className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 transition-colors">
                    ...
                    <table className="w-full min-w-max">
                        <thead>
                            <tr className="border-b border-slate-100">
                                <th className="text-left px-5 py-2 text-[11px] font-black text-slate-400 uppercase tracking-wider w-32 sticky left-0 bg-white">Member</th>
                                {dates.map(d => {
                                    const ds = fmtDate(d);
                                    ...
                                    return (
                                        <th key={ds} className={`px-1 py-2 text-center min-w-[44px] ${isWeekend ? "bg-slate-50" : ""}`}>
                                            <div className={`text-[11px] font-black ${isToday ? "text-brand-blue" : "text-slate-400"}`}>
                                                {d.toLocaleDateString("en", { weekday: "short" })}
                                            </div>
                                            <div className={`text-[11px] font-black mt-0.5 ${isToday ? "text-brand-blue" : "text-slate-500"}`}>
                                                {d.getDate()}
                                            </div>
                                        </th>
                                    );
                                    ...
                                    <td className="px-5 py-2 sticky left-0 bg-white">
                                        <div className="flex items-center gap-2">
                                            {member.avatar_url ? (
                                                ...
                                                <div className="w-6 h-6 rounded-full bg-brand-blue/10 flex items-center justify-center text-[11px] font-black text-brand-blue flex-shrink-0">
                                    ...
                                    {dates.map(d => {
                                        ...
                                        return (
                                            <td key={ds} className={`px-1 py-1.5 text-center ${isWeekend ? "bg-slate-50/60" : ""}`}>
                                                <div className={`mx-auto w-8 h-8 rounded-lg border flex flex-col items-center justify-center transition-colors ${isToday ? "ring-1 ring-brand-blue" : ""} ${heatColor(load.count)}`}
                                                    title={`${load.count} tasks, ${load.hours}h`}>
                                                    {load.count > 0 && (
                                                        <span className={`text-[11px] font-black ${heatLabel(load.count)}`}>{load.count}</span>
                                                    )}
                                                </div>
                                            </td>
                                        );
                ...
                <div className="flex items-center gap-4 px-5 py-3 border-t border-slate-50">
                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-wider">Load:</span>
