    ...
    return (
        <div className="flex h-[800px] bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden font-sans">
            ...
            <div className="flex-1 flex flex-col min-w-0 relative">
                            ...
                            {dateWindow.map((d, i) => {
                                ...
                                return (
                                    <div key={i} style={{ width: cellW }} className={`h-full border-r border-slate-100 flex flex-col items-center pt-2 ${isWeekend ? "bg-slate-50/50" : ""}`}>
                                        <div className={`text-[10px] font-black tracking-widest uppercase mb-1 ${isToday ? "text-[#0c64ef]" : "text-slate-400"}`}>
                                            {d.toLocaleDateString("en-US", { weekday: "short" })}
                                        </div>
                                        <div className={`text-sm font-black ${isToday ? "text-[#0c64ef]" : "text-[#1d1d1f]"}`}>
                                    ...
                                    <div key={task.id} className="relative h-[60px] mb-3 group cursor-pointer" onClick={() => handleTaskClick(task)}>
                                        <div
                                            className={`absolute top-1/2 -translate-y-1/2 h-10 rounded-full flex items-center px-2 transition-all duration-300 overflow-hidden
                                                ${colorClass} 
                                                ${isActive ? "ring-4 ring-offset-2 ring-[#0c64ef]/20 shadow-lg scale-[1.02] z-20" : "shadow-sm hover:shadow-md hover:brightness-105 z-10"}`}
                                            style={{
                ...
                {/* FAB */}
                <button className="absolute bottom-8 right-8 w-14 h-14 bg-[#0c64ef] rounded-full flex items-center justify-center shadow-xl shadow-[#0c64ef]/30 hover:scale-105 transition-transform z-30 group">
                    <Plus className="text-white group-hover:rotate-90 transition-transform duration-300" size={24} />
                    <span className="absolute -top-6 text-[10px] font-black text-[#0c64ef] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Add new</span>
                </button>
            </div>
            ...
            <div className="w-[320px] bg-slate-50/50 border-l border-slate-100 p-6 flex flex-col gap-6 overflow-y-auto">
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 mt-auto relative overflow-hidden group">
                    ...
                    <div className="text-center mb-4">
                        <div className="text-4xl font-black text-[#0c64ef] tracking-tighter">
                    ...
                    <div className="h-2.5 rounded-full bg-slate-100 overflow-hidden relative">
                        <div 
                            className="absolute top-0 bottom-0 left-0 bg-[#0c64ef] rounded-full transition-all duration-1000 ease-out"
                            style={{ width: `${progress}%` }}
                        />
                    </div>
                    <div className="text-right mt-2 text-[9px] font-black text-[#0c64ef]">{Math.round(progress)}%</div>
                </div>
            </div>
        </div>
    );
