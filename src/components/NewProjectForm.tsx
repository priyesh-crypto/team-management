  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Card className="p-8 space-y-6 bg-white/80 backdrop-blur-xl border-none shadow-[0_20px_60px_rgba(0,0,0,0.05)]">
        <div className="space-y-2">
          ...
          <Input 
            ...
            className="h-14 text-lg font-bold bg-[#f5f5f7] border-none focus:ring-2 focus:ring-brand-blue transition-all"
            required
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <textarea 
            ...
            className="w-full min-h-[100px] p-4 rounded-2xl bg-[#f5f5f7] border-none text-sm font-medium focus:ring-2 focus:ring-brand-blue transition-all resize-none"
          />
        </div>
          ...
          <div className="flex gap-4 px-2">
            {PROJECT_COLORS.map(color => (
              <button
                ...
                className={`w-8 h-8 rounded-full transition-all duration-300 ring-offset-4 ${form.color === color ? 'ring-2 ring-brand-blue scale-110' : 'hover:scale-105 opacity-60 hover:opacity-100'}`}
      ...
      <div className="flex gap-4 pt-4">
        ...
        <Button 
          type="submit" 
          className="flex-[2] h-14 rounded-2xl font-black uppercase tracking-widest text-[11px] bg-brand-blue shadow-[0_8px_30px_rgba(0,113,227,0.3)] hover:shadow-[0_12px_40px_rgba(0,113,227,0.4)] transition-all"
          disabled={loading}
        >
