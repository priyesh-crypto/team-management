...
export function ProjectSwitcher({ projects, userRole }: { projects: Project[], userRole?: 'employee' | 'manager' }) {
  ...
  return (
    <div className="flex flex-col gap-1.5 mt-6 px-1">
      ...
      >
        <LayoutGrid size={16} strokeWidth={2.5} className={cn("transition-colors", !projectId ? "text-brand-blue" : "text-slate-400 group-hover:text-slate-800")} />
        <span>Main Inbox</span>
        {!projectId && (
            <div className="absolute left-0 w-1 h-5 bg-brand-blue rounded-r-full" />
        )}
      </button>
