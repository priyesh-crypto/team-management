...
export function Button({
    ...
    const styles: Record<string, string> = {
        primary:
            "bg-brand-blue text-white hover:bg-[#0950c4]",
        secondary:
...
export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
    return (
        <select
            {...props}
            className={`w-full px-3 py-2 rounded-md border border-[#e5e5ea] bg-white text-sm text-[#1d1d1f] focus:outline-none focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/10 transition-colors ${
                props.className ?? ""
            }`}
        />
    );
