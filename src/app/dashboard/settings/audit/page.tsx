    ...
    if (!entitlement || !hasFeature(entitlement, "org_audit_log")) {
        return (
            <div className="p-10 max-w-2xl mx-auto text-center">
                ...
                <p className="text-slate-500">Available on the Business plan. <a href="/dashboard/settings/billing" className="text-brand-blue font-bold hover:underline">Upgrade</a> to unlock.</p>
            </div>
        );
    }
            ...
            <div className="bg-white rounded-2xl border border-slate-100 divide-y divide-slate-50 overflow-hidden">
                {(logs ?? []).map(log => (
                    <div key={log.id} className="flex items-start gap-4 px-5 py-4 hover:bg-slate-50/50 transition-colors">
                        <div className="w-9 h-9 rounded-xl bg-brand-blue/10 flex items-center justify-center text-base flex-shrink-0">
